...
func (mw *MainWindow) Update() error {
	...
	for _, obj := range objects {
		...
		id := obj.GetID()
		aliveIDs[id] = true
		if _, exists := mw.SpriteMap[id]; !exists { mw.createVegetableSprite(obj) }
	...
	for id := range mw.SpriteMap {
		if !aliveIDs[id] {
			...
			if mw.SelectedAgent != nil && mw.SelectedAgent.GetID() == id { mw.SelectedAgent = nil }
		}
	}
	return nil
}
...
func (mw *MainWindow) handleGameClick(x, y float64) {
	...
	for _, a := range agents {
		...
		if _, isHuman := a.(*simulation.Human); !isHuman { continue }
...
func (mw *MainWindow) Draw(screen *ebiten.Image) {
	...
	if mw.SelectedAgent != nil {
		...
		ebitenutil.DrawRect(mw.GameView, pos.X-5, pos.Y-25, 10, 10, color.White)
	}
...
func (mw *MainWindow) drawSidebarInfo(screen *ebiten.Image) {
	...
	if len(stats) > 0 {
		...
		y := 20
		line := 20
		ebitenutil.DebugPrintAt(screen, "--- STATISTIQUES ---", 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Temps: %d", last.Tick), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Vitesse: x%d", mw.SpeedSlider.Current), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Humains: %d", last.HumansAlive), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Animaux: %d", last.AnimalsAlive), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Plantes: %d", last.VegetablesAlive), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Pragmatiques: %d", last.CountPragmatic), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Prudents: %d", last.CountCautious), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Egoïstes: %d", last.CountSelfish), 10, y); y+=line
	...
	y := 250
	line := 20
	ebitenutil.DebugPrintAt(screen, "--- INSPECTION ---", 10, y); y+=line
	if mw.SelectedAgent != nil {
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("ID: %d", mw.SelectedAgent.GetID()), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Nom: %s", mw.SelectedAgent.GetName()), 10, y); y+=line
		ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Vie: %d", mw.SelectedAgent.GetHealth()), 10, y); y+=line
		...
		if h, ok := mw.SelectedAgent.(*simulation.Human); ok {
			...
			prof := "Inconnu"
			switch h.GetProfile() {
				case simulation.Pragmatic: prof = "Pragmatique"
				case simulation.Cautious: prof = "Prudent"
				case simulation.Selfish: prof = "Egoïste"
				case simulation.Collectivist: prof = "Collectiviste"
			}
			ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Type: %s", prof), 10, y); y+=line
			
			action := "Rien"
			if h.GetCurrentAction() != nil {
				switch h.GetCurrentAction().(type) {
				case *simulation.RestAction: action = "Dort"
				case *simulation.GatherAction: action = "Cueille"
				case *simulation.HuntAction: action = "Chasse"
				case *simulation.ReproduceAction: action = "Reproduction"
				}
			}
			ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Action: %s", action), 10, y); y+=line
		}
	} else {
...
func (mw *MainWindow) createVegetableSprite(obj simulation.Object) {
	if veg, ok := obj.(*simulation.Vegetable); ok {
		var s Sprite
		switch veg.GetType() {
		case simulation.Carrot: s = NewVegetableSprite(10, 10, 255, 165, 0)
		case simulation.Lettuce: s = NewVegetableSprite(12, 12, 50, 205, 50)
		case simulation.Berry: s = NewVegetableSprite(8, 8, 148, 0, 211)
		}
		if s != nil {
...
func (mw *MainWindow) updateAgentAnimation(agent simulation.Agent, s Sprite, dx, dy float64) {
	...
	case *HumanSprite:
		if !agent.IsAlive() { sTyped.SetAnimationRow(16); return }
		if isMoving {
			baseRow := 4
			if math.Abs(dx) > math.Abs(dy) {
				if dx > 0 { sTyped.SetAnimationRow(baseRow + 2) } else { sTyped.SetAnimationRow(baseRow + 1) }
			} else {
				if dy > 0 { sTyped.SetAnimationRow(baseRow + 0) } else { sTyped.SetAnimationRow(baseRow + 3) }
			}
		} else { sTyped.SetAnimationRow(0) }
	case *CowSprite:
		if isMoving {
			if dx > 0 { sTyped.SetAnimationRow(1) } else { sTyped.SetAnimationRow(0) }
		}
	case *RoosterSprite:
		if isMoving {
			if dx > 0 { sTyped.SetAnimationRow(1) } else { sTyped.SetAnimationRow(0) }
...
func (mw *MainWindow) createSpriteForAgent(agent simulation.Agent) {
	var s Sprite
	switch v := agent.(type) {
	case *simulation.Human: s = NewHumanSprite()
	case *simulation.Animal:
		switch v.GetType() {
		case simulation.Chicken: s = NewRoosterSprite()
		case simulation.Cow: s = NewCowSprite()
		case simulation.Bull: s = NewBullSprite()
		}
	}
	if s != nil {
